...
| | |
|---|---|
| Coordinator **nametag** | `<coordinator-nametag>` — *preferred*: address it by name, resolved to the npub below via the Sphere SDK (global + authenticated) |
| Coordinator npub | `<coordinator-npub — shared privately with authorized peers>` — the fallback / first‑boot value the nametag resolves to |
...
> nametag **`<coordinator-nametag>`**, so anywhere a `<coordinator-npub>` is expected you can pass
> **`<coordinator-nametag>`** instead — `/consult-coordinator <coordinator-nametag> "…"`, `remote-coord.sh
> emit … --to <coordinator-nametag>`, etc. The nametag resolves to the coordinator's npub globally
...
> re‑resolve manually: `node lib/sphere-helper.mjs resolve-nametag <coordinator-nametag>`.
...
# in your Claude session (skills) — pass the nametag `<coordinator-nametag>` or the coord npub
/recall-prior-work <keywords>                 # research before claiming
/consult-coordinator <coordinator-nametag> "<intent>"  # broadcast + claim + consult (nametag or npub)
...
bash $RC consult-open --to <coordinator-nametag> --intent "…" --areas "…" --changes "…" --questions "…"
bash $RC emit "$(bash $RC envelope <kind> --payload '…')" --to <coordinator-nametag>
```
