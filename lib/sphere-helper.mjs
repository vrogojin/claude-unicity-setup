...
// --- NIP-59 gift-wrap query window -------------------------------------------------
// NIP-17 DMs are NIP-59 gift-wrapped (kind 1059); the spec randomizes the OUTER wrap's
// created_at BACKWARD by up to ~2 days (and sometimes slightly into the future) to hide
// timing metadata. A relay filters on that outer created_at, so a message really sent at
// 12:05 can carry a wrap stamped 09:16 — and an advancing `since` cursor will PERMANENTLY
// exclude it (silent, permanent receive-side loss). The fix: never let the gift-wrap query
// floor advance past `now - LOOKBACK`, so every poll re-offers at least the last LOOKBACK of
// wraps; the daemon dedups by outer wrap id, so the wide window never re-delivers.
//
// LOOKBACK = 3 days > NIP-59's 2-day max backdating + a day of margin for cursor drift
// (the old value was exactly 2 days, which left zero margin once the cursor advanced past a
// message's real send time). Env-overridable; falls back to the legacy GIFTWRAP_SKEW_SECONDS
// name so an existing deployment that tuned it keeps its value.
export const GIFTWRAP_LOOKBACK_SECONDS = (() => {
...
// Effective gift-wrap query floor: the rolling `now - LOOKBACK` horizon, but honoring an
// explicitly-smaller `since` (e.g. `--since 0` for a full manual drain, or a far-behind
// daemon cursor catching up after downtime). Never returns < 0.
export function giftwrapSince(since, nowSec = Math.floor(Date.now() / 1000), lookbackSecs = GIFTWRAP_LOOKBACK_SECONDS) {
  return Math.max(0, Math.min(since, nowSec - lookbackSecs));
}
...
// Resolves to the array of events. If a `diag` object is passed, it is populated with
// reachability signals so callers can tell "relay reachable, authoritatively empty" from
// "relay never answered": diag.connected (ws opened), diag.eose (relay sent EOSE → its
// answer is complete), diag.timedOut (backstop fired before EOSE). This is what lets the
// ticket layer distinguish a missing/expired ticket from an unreachable relay.
function fetchEventsRaw(relayUrl, filters, timeoutMs = 6000, diag) {
  return new Promise((resolvePromise) => {
    ...
    const finish = (timedOut) => {
      ...
      settled = true;
      clearTimeout(timer);
      if (diag) diag.timedOut = !!timedOut;
      try { ws.close(); } catch {}
    ...
    ws.onmessage = (e) => {
      ...
      if (a[0] === 'EVENT' && a[2] && a[2].id) {
        ...
        open.delete(a[1]);
        if (diag) diag.eose = true;   // relay acknowledged the subscription → its answer is authoritative
        if (open.size === 0) finish(false);
      }
    };
...
async function checkMessages(args) {
  ...
  // Gift-wrap query floor: rolling `now - LOOKBACK`, honoring an explicit smaller
  // --since (see giftwrapSince / GIFTWRAP_LOOKBACK_SECONDS above). This never lets an
  // advancing cursor exclude a backdated NIP-59 wrap; the daemon dedups by outer event
  // id so the wide window can't re-deliver.
  const dmSince = giftwrapSince(since);
  ...
  try {
    ...
    for (const event of rawById.values()) {
      if (event.kind === nostr.GIFT_WRAP) {
        try {
          // unwrap (NIP17.unwrap) verifies the SEAL signature — the message's SENDER and
          // CONTENT are authenticated and cannot be forged. It does NOT verify the outer
          // kind:1059 wrap (signed by a throwaway ephemeral key), so a malicious relay could
          // replay the same ciphertext under a fresh wrap (new ephemeral key ⇒ new outer id)
          // to bypass our outer-id dedup. That only causes duplicate DELIVERY / seen-set
          // churn, never forged content; the seen-set is time-bounded (see sphere-daemon), and
          // the authoritative idempotency is downstream in classify-inbound.sh, which dedups
          // on sha1(from|body) — stable across re-wraps because from+body are unchanged.
          ...
          messages.push({
            ...
            // OUTER wrap created_at (unix sec) — the value the relay filters `since` on.
            // The daemon keys its seen-set prune on this so an id is retained exactly as
            // long as the relay can still re-offer it (NIP-59 skews it by up to ±2 days).
        ...
        messages.push({
          ...
          wrap_created_at: event.created_at, // plain (non-wrapped) event created_at
...
// exponential backoff per relay; a fresh `since = now − LOOKBACK` on each (re)connect
...
async function watchMessages(args) {
  ...
  const handleEvent = (ev) => {
    ...
    if (ev.kind === nostr.GIFT_WRAP) {
      ...
      emit({
        ...
        wrap_created_at: ev.created_at, // OUTER wrap created_at (see checkMessages)
      ...
      emit({
        ...
        wrap_created_at: ev.created_at, // plain (non-wrapped) event created_at
  ...
  const connect = (relayUrl) => {
    ...
    const open = () => {
      ...
      let settled = false;
      let hbTimer = null;
      let lastActivity = Date.now();   // any inbound frame (EVENT/EOSE/CLOSED/NOTICE) refreshes this
      let awaitingProbe = false;       // a heartbeat REQ is outstanding with no reply yet
      const nowSec = Math.floor(Date.now() / 1000);
      const filters = [{ kinds: [nostr.GIFT_WRAP], '#p': [myPubkeyHex], since: giftwrapSince(nowSec, nowSec) }];
      ...
      const reconnect = (why) => {
        ...
        console.error(`[sphere-helper] watch: ${relayUrl} ${why || 'disconnected'} — reconnecting in ${wait}ms`);
        setTimeout(open, wait);
      };
      try { ws = new WebSocket(relayUrl); } catch { reconnect('connect threw'); return; }
      ws.onopen = () => {
        ...
        // KEEPALIVE (L2.2): a half-open TCP socket (NAT/idle timeout, relay restart without FIN)
        // fires no onclose/onerror — the watcher would go silently deaf forever. Every 60s send a
        // cheap probe REQ; a healthy relay answers (EVENT/EOSE) and refreshes lastActivity, a dead
        // one answers nothing → force-close → the reconnect path takes over. (The daemon's 60s poll
        // belt already bounds the delivery gap; this restores sub-second delivery quickly.)
        stopHb();
        hbTimer = setInterval(() => {
          if (settled) return;
          if (awaitingProbe && (Date.now() - lastActivity) > 90000) {
            reconnect('no relay activity within 90s of heartbeat probe'); return;
          }
          try {
            ws.send(JSON.stringify(['REQ', 'whb', { kinds: [nostr.GIFT_WRAP], '#p': [myPubkeyHex], limit: 1 }]));
            awaitingProbe = true;
          } catch { reconnect('heartbeat send failed'); }
        }, 60000);
      };
      ws.onmessage = (e) => {
        lastActivity = Date.now(); awaitingProbe = false; // ANY frame is proof the link is alive
        let a;
        try { a = JSON.parse(typeof e.data === 'string' ? e.data : e.data.toString()); } catch { return; }
        if (a[0] === 'EVENT' && a[2]) handleEvent(a[2]);
        else if (a[0] === 'CLOSED') {
          // NIP-01 CLOSED for one of OUR sub ids = the relay dropped our subscription. The old
          // code ignored CLOSED, so a relay-side drop caused PERMANENT silent deafness. Treat it
          // as a reconnect trigger.
          const sid = typeof a[1] === 'string' ? a[1] : '';
          if (sid && sid[0] === 'w') reconnect(`relay CLOSED sub ${sid}: ${a[2] || ''}`);
        }
        // EOSE is still ignored for delivery — the socket stays open for live events — but it
        // already refreshed lastActivity above, which is exactly what keeps the keepalive honest.
      };
      ws.onerror = () => reconnect('socket error');
      ws.onclose = () => reconnect('socket closed');
    };
    open();
  };
...
async function relayPublish(args) {
  ...
  const result = await new Promise((resolvePromise) => {
    ...
    ws.onopen = () => { try { ws.send(JSON.stringify(['EVENT', ev])); } catch (e) { done({ ok: false, reason: e.message }); } };
    ws.onmessage = (e) => {
      let a;
      try { a = JSON.parse(typeof e.data === 'string' ? e.data : e.data.toString()); } catch { return; }
      if (a[0] === 'OK' && a[1] === ev.id) { clearTimeout(timer); done({ ok: a[2] === true, reason: a[3] || '' }); }
    };
...
async function relayFetch(args) {
  ...
  const relay = args.relay || 'wss://nostr-relay.testnet.unicity.network';
  const diag = { connected: false, eose: false, timedOut: false };
  const events = await fetchEventsRaw(relay, [{ kinds: [TICKET2_KIND], '#d': [dtag.toLowerCase()] }], 6000, diag);
  // reachable = the WebSocket actually opened; eose = the relay gave a complete answer
  // (so 0 events is authoritative "no such event", not "we couldn't reach the relay").
  output({ events, relay, reachable: diag.connected, eose: diag.eose, timedOut: diag.timedOut });
}

// --- Main dispatch ---

// Only run the CLI when executed directly (`node sphere-helper.mjs …`), not when a test
// imports this module for its pure helpers (giftwrapSince, …). realpath both sides so a
...
Commands:
  ...
  relay-publish --relay <url>  (stdin: event JSON)        Publish an event, wait for relay OK
  relay-fetch --relay <url> --dtag <hex>                  Fetch kind-30777 events by #d
