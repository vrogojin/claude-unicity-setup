...
async function loadSdk() {
  try {
    return await import('@unicity/sphere-sdk');
  } catch {
    fail('@unicity/sphere-sdk is not installed.\nInstall it with: npm install @unicity/sphere-sdk');
  }
}
...
async function createIdentity() {
  const { Identity } = await loadSdk();

  try {
    const identity = Identity.create ? Identity.create() : Identity.fromMnemonic();
