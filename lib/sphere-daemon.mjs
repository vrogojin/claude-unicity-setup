...
function saveSeen(projectDir, ids) {
  // Atomic write (temp + rename): a crash/kill mid-write must never leave a truncated,
  // unparseable seen-events.json — that would make loadSeen() fall back to [] and
  // re-dispatch the entire backlog on the next start.
  try {
    const file = seenFileFor(projectDir);
    const tmp = `${file}.tmp.${process.pid}`;
    writeFileSync(tmp, JSON.stringify(ids.slice(-SEEN_MAX)));
    renameSync(tmp, file);
  } catch (e) {
...
async function startDaemon(projectDir, intervalSecs, liveMode = false) {
  ...
  if (liveMode) {
    ...
    const startWatcher = () => {
      ...
      let buf = '';
      // Decode the NDJSON byte stream as UTF-8 with a StringDecoder so a multi-byte
      // character split across two chunk boundaries is not corrupted into replacement
      // characters (which would break JSON.parse of that line and drop a message).
    ...
    poll();
    startWatcher();
    // Safety net: keep a LOW-FREQUENCY poll running even while the live watcher is up.
    // The watcher's own idle-watchdog recycles a half-open socket, but this poll is the
    // belt-and-suspenders guarantee — a wedged-but-alive watcher can never blackhole
    // delivery, because dispatch() dedups so the poll and the watcher can't double-send.
    const safetySecs = Math.max(intervalSecs, 120);
    log(`Live mode: safety poll every ${safetySecs}s (in addition to the watcher)`);
    setInterval(poll, safetySecs * 1000);
  } else {
    poll();
    startInterval();
  }
