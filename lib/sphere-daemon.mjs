...
async function startDaemon(projectDir, intervalSecs) {
  ...
  // Dedup state (persisted): insertion-ordered id list + fast lookup set.
  ...
  const poll = () => {
    ...
    // Only dispatch messages we haven't already handled. Messages without an id
    // (older helper / malformed) can't be deduped, so they always dispatch.
    const newMessages = allMessages.filter((msg) => !(msg.id && seenSet.has(msg.id)));
    ...
    if (dirty) {
      // Bound the in-memory set too, mirroring the persisted cap.
